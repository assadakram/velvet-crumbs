export const BROWNIES = [
  {
    id: 'brownie-choco-hazelnut',
    img: '/images/choco_hazelnut_brownie.png',
    price: 4.50,
    origPrice: 4.50,
    special: false,
    color: 'from-amber-700 to-amber-900',
    name: { en: 'Choco Hazelnut Brownie', fi: 'Hasselpähkinä-Suklaa Brownie' },
    desc: {
      en: 'A rich, melt-in-your-mouth chocolate brownie topped with roasted hazelnuts and a soft Nutella-inspired hazelnut spread. Baked with premium ingredients for the ultimate gooey texture.',
      fi: 'Täyteläinen ja suussa sulava suklaabrownie, jonka päällä on paahdettuja hasselpähkinöitä ja pehmeää Nutella-tyylistä hasselpähkinälevitettä. Leivottu laadukkaista raaka-aineista täydellisen mehevän rakenteen saavuttamiseksi.'
    },
    ingredients: {
      en: ['Cane sugar', 'Wheat flour', 'Vegetable oil', 'Water', 'Dark cocoa powder', 'Hazelnut spread', 'Roasted hazelnuts', 'Chocolate chips', 'Powdered sugar', 'Ground flaxseed', 'Vanilla extract', 'Salt'],
      fi: ['Ruokosokeri', 'Vehnäjauho', 'Kasviöljy', 'Vesi', 'Tumma kaakaojauho', 'Hasselpähkinälevite', 'Paahdettu hasselpähkinä', 'Suklaahippu', 'Tomusokeri', 'Hienonnettu pellavansiemen', 'Vanilja-uute', 'Suola']
    }
  },
  {
    id: 'brownie-pistachio',
    img: '/images/Pistachio_fudge_brownie.png',
    price: 4.50,
    origPrice: 4.50,
    special: false,
    color: 'from-green-600 to-green-800',
    name: { en: 'Pistachio Fudge Brownie', fi: 'Pistaasi-Fudge Brownie' },
    desc: {
      en: 'A decadent brownie base crowned with premium, smooth pistachio cream and sprinkled with crushed pistachios. Plant-based luxury in every bite.',
      fi: 'Ylellinen brownie-pohja, joka on kruunattu ensiluokkaisella, pehmeällä pistaasikermalla ja ripoteltu murskatuilla pistaaseilla. Kasvipohjaista luksusta jokaisella suupalalla.'
    },
    ingredients: {
      en: ['Vegan cane sugar', 'Wheat flour', 'Vegetable oil', 'Water', 'Dark cocoa powder', 'Pistachio spread', 'Crushed pistachios', 'Vegan chocolate chips', 'Powdered sugar', 'Ground flaxseed', 'Vanilla extract', 'Salt'],
      fi: ['Vegaaninen ruokosokeri', 'Vehnäjauho', 'Kasviöljy', 'Vesi', 'Tumma kaakaojauho', 'Pistaasilevite', 'Murskattu pistaasi', 'Vegaaninen suklaahippu', 'Tomusokeri', 'Hienonnettu pellavansiemen', 'Vanilja-uute', 'Suola']
    }
  },
  {
    id: 'brownie-biscoff',
    img: '/images/Biscoff_melt_brownie.png',
    price: 4.50,
    origPrice: 4.50,
    special: false,
    color: 'from-amber-600 to-amber-800',
    name: { en: 'Biscoff Melt Brownie', fi: 'Biscoff-Melt Brownie' },
    desc: {
      en: 'A gooey dark chocolate brownie topped with a generous layer of melted Lotus Biscoff spread and a crunchy Biscoff biscuit. A 100% vegan treat crafted from premium ingredients.',
      fi: 'Mehevä tummasuklaabrownie, jonka päällä on reilu kerros sulatettua Lotus Biscoff -levitettä ja rouskuva Biscoff-keksi. 100 % vegaaninen herkku laadukkaista raaka-aineista.'
    },
    ingredients: {
      en: ['Vegan cane sugar', 'Wheat flour', 'Vegetable oil', 'Water', 'Dark cocoa powder', 'Biscoff spread', 'Biscoff biscuit', 'Vegan chocolate chips', 'Powdered sugar', 'Ground flaxseed', 'Vanilla extract', 'Salt'],
      fi: ['Vegaaninen ruokosokeri', 'Vehnäjauho', 'Kasviöljy', 'Vesi', 'Tumma kaakaojauho', 'Biscoff-levite', 'Biscoff-keksi', 'Vegaaninen suklaahippu', 'Tomusokeri', 'Hienonnettu pellavansiemen', 'Vanilja-uute', 'Suola']
    }
  },
  {
    id: 'brownie-coconut',
    img: '/images/Coconut_crush_brownie.png',
    price: 4.50,
    origPrice: 4.50,
    special: false,
    color: 'from-stone-300 to-stone-500',
    name: { en: 'Coconut Crush Brownie', fi: 'Kookos-Crush Brownie' },
    desc: {
      en: 'Velvety vegan white chocolate spread and coconut flakes generously layered over a fudgy dark chocolate base. Made from high-quality plant-based ingredients for a soft, dreamy bite.',
      fi: 'Samettista vegaanista valkosuklaalevitettä ja kookoshiutaleita mehukkaan tummasuklaapohjan päällä. Valmistettu laadukkaista kasvipohjaisista aineksista pehmeän unelmalliseksi suupalaksi.'
    },
    ingredients: {
      en: ['Vegan cane sugar', 'Wheat flour', 'Vegetable oil', 'Water', 'Dark cocoa powder', 'Vegan white chocolate spread', 'Coconut flakes', 'Vegan chocolate chips', 'Powdered sugar', 'Ground flaxseed', 'Vanilla extract', 'Salt'],
      fi: ['Vegaaninen ruokosokeri', 'Vehnäjauho', 'Kasviöljy', 'Vesi', 'Tumma kaakaojauho', 'Vegaaninen valkosuklaalevite', 'Kookoshiutale', 'Vegaaninen suklaahippu', 'Tomusokeri', 'Hienonnettu pellavansiemen', 'Vanilja-uute', 'Suola']
    }
  },
  {
    id: 'brownie-oreo',
    img: '/images/Oreo_crunch_brownie.png',
    price: 4.50,
    origPrice: 4.50,
    special: false,
    color: 'from-slate-700 to-slate-900',
    name: { en: 'Oreo Crunch Brownie', fi: 'Oreo-Crunch Brownie' },
    desc: {
      en: 'Our ultra-fudgy plant-based brownie swirled with creamy vegan white chocolate spread and loaded with crushed Oreos. Premium ingredients meet classic cookies-and-cream goodness.',
      fi: 'Erittäin täyteläinen kasvipohjainen brownie, jossa yhdistyvät kermainen vegaaninen valkosuklaalevite ja rapea Oreo-keksimurska. Ensiluokkaiset raaka-aineet kohtaavat klassisen keksi-kermakombinaation.'
    },
    ingredients: {
      en: ['Vegan cane sugar', 'Wheat flour', 'Vegetable oil', 'Water', 'Dark cocoa powder', 'Vegan white chocolate spread', 'Oreo cookies', 'Vegan chocolate chips', 'Powdered sugar', 'Ground flaxseed', 'Vanilla extract', 'Salt'],
      fi: ['Vegaaninen ruokosokeri', 'Vehnäjauho', 'Kasviöljy', 'Vesi', 'Tumma kaakaojauho', 'Vegaaninen valkosuklaalevite', 'Oreo-keksi (murskattu)', 'Vegaaninen suklaahippu', 'Tomusokeri', 'Hienonnettu pellavansiemen', 'Vanilja-uute', 'Suola']
    }
  },
  {
    id: 'brownie-raspberry',
    img: '/images/raspberry_delight_brownie.png',
    price: 4.50,
    origPrice: 4.50,
    special: false,
    color: 'from-rose-500 to-rose-700',
    name: { en: 'Raspberry Delight Brownie', fi: 'Vadelma-Delight Brownie' },
    desc: {
      en: 'Rich vegan dark chocolate combined with smooth vegan white chocolate spread and tangy freeze-dried raspberries. Crafted from high-grade ingredients for a bright, melt-in-your-mouth flavor.',
      fi: 'Täyteläinen vegaaninen tummasuklaa kohtaa pehmeän vegaanisen valkosuklaalevitteen ja raikkaan pakastekuivatun vadelman. Valmistettu laadukkaista aineksista raikkaan ja suussa sulavan maun luomiseksi.'
    },
    ingredients: {
      en: ['Vegan cane sugar', 'Wheat flour', 'Vegetable oil', 'Water', 'Dark cocoa powder', 'Vegan white chocolate spread', 'Freeze-dried raspberries', 'Vegan chocolate chips', 'Powdered sugar', 'Ground flaxseed', 'Vanilla extract', 'Salt', 'Food coloring'],
      fi: ['Vegaaninen ruokosokeri', 'Vehnäjauho', 'Kasviöljy', 'Vesi', 'Tumma kaakaojauho', 'Vegaaninen valkosuklaalevite', 'Pakastekuivattu vadelma', 'Vegaaninen suklaahippu', 'Tomusokeri', 'Hienonnettu pellavansiemen', 'Vanilja-uute', 'Suola', 'Elintarvikeväri']
    }
  }
];

export const BOXES = [
  {
    id: 'box-ultimate-brownie',
    img: '/images/Ultimate_Brownie_Box.png',
    price: 19.90,
    origPrice: 27.00,
    special: false,
    color: 'from-orange-500 to-red-600',
    name: { en: 'Ultimate Brownie Box', fi: 'Ultimate Brownie -Lajitelmaboksi' },
    desc: {
      en: 'Treat yourself to a medium-sized piece of all six signature brownie flavors: Biscoff Melt, Hazelnut & Walnut, Pistachio, Coconut & White Chocolate, Raspberry & White Chocolate, and Oreo & White Chocolate. Leave a note in Special Instructions at checkout to fully customize the flavors.',
      fi: 'Sisältää keskikokoisen palan kaikkia kuutta suosikkimakuamme. Voit koota boksin maut mieleiseksesi kirjoittamalla toiveesi kassalla kohtaan "Erityisohjeet".'
    },
    ingredients: {
      en: ['Assorted brownie ingredients'],
      fi: ['Lajitelma brownie-ainesosia']
    }
  },
  {
    id: 'box-party-mini',
    img: '/images/Brownie_part_box.png',
    price: 24.99,
    origPrice: 30.00,
    special: false,
    color: 'from-fuchsia-500 to-pink-600',
    name: { en: 'Party Mini Brownie Box (25 pcs)', fi: 'Party Mini Brownie -Boksi (25 kpl)' },
    desc: {
      en: '25 bite-sized mini brownies featuring an assortment of our rich signature flavors. Perfect for birthdays or office events. Leave a note in Special Instructions to customize your flavor selection.',
      fi: '25 suupalan kokoista minibrownieta suosikkimakuja. Täydellinen juhliin! Räätälöi maut kassalla "Erityisohjeet"-kohdassa.'
    },
    ingredients: {
      en: ['Assorted brownie ingredients'],
      fi: ['Lajitelma brownie-ainesosia']
    }
  },
  {
    id: 'box-mini-assortment',
    img: '/images/Mini_Brownie_&_Cookie_Assortment_Box.jpg',
    price: 24.99,
    origPrice: 30.00,
    special: false,
    color: 'from-teal-500 to-emerald-600',
    name: { en: 'Mini Brownie & Cookie Assortment Box (20 pcs)', fi: 'Mini Brownie & Keksi -Lajitelmaboksi (20 kpl)' },
    desc: {
      en: '10 bite-sized mini brownies and 10 mini cookies. Comes packed with a delicious surprise mix of signature flavors. Leave a note in Special Instructions to customize.',
      fi: '10 minibrownieta ja 10 minikeksiä herkullisena yllätyssekoituksena. Kokoa sisältö mieleiseksi "Erityisohjeet"-kohdassa kassalla.'
    },
    ingredients: {
      en: ['Assorted cookie and brownie ingredients'],
      fi: ['Lajitelma keksien ja brownieiden ainesosia']
    }
  },
  {
    id: 'box-mini-cookie',
    img: '/images/Mini_cookie_box.jpg',
    price: 19.99,
    origPrice: 24.00,
    special: false,
    color: 'from-amber-400 to-orange-500',
    name: { en: 'Mini Cookie Box (16 pcs)', fi: 'Mini Keksi -Boksi (16 kpl)' },
    desc: {
      en: '16 soft mini cookies (45g each). Includes 4 of each: Chocolate Indulgence, Hazelnut Chocolate Chip, Caramel Gold, and Ruby Velvet Bliss. Flavor swaps? Just leave a note in Special Instructions!',
      fi: '16 pehmeää minikeksiä (45 g). Sisältää 4 kpl kutakin: Chocolate Indulgence, Hazelnut Chocolate Chip, Caramel Gold ja Ruby Velvet Bliss. Vaihdot onnistuvat "Erityisohjeet"-kohdassa.'
    },
    ingredients: {
      en: ['Assorted cookie ingredients'],
      fi: ['Lajitelma keksien ainesosia']
    }
  },
  {
    id: 'box-mix-match',
    img: '/images/Mix_and_Match_duo.png',
    price: 14.90,
    origPrice: 18.00,
    special: false,
    color: 'from-indigo-400 to-purple-600',
    name: { en: 'Mix & Match Duo Box', fi: 'Mix & Match Duo -Boksi' },
    desc: {
      en: 'Enjoy 2 Large Cookies (140g each) and 2 Medium Brownies in the flavors of your choice. Write your flavor selections in the Special Instructions section at checkout!',
      fi: 'Nauti 2 isosta keksistä ja 2 keskikokoisesta browniesta valitsemissasi mauissa. Kirjoita toiveesi kassalla "Erityisohjeet"-kohtaan!'
    },
    ingredients: {
      en: ['Assorted cookie and brownie ingredients'],
      fi: ['Lajitelma keksien ja brownieiden ainesosia']
    }
  }
];
